...
from Focal_Loss import EFocalLoss,FocalLoss
...
def main():
    global args, best_prec1, train_list, experiment_dir, best_loss,FL,iters_per_epoch
    args = parser.parse_args()
    ...
    #Focal loss の設定
    FL = EFocalLoss(class_num=2)
    # データ数/バッチサイズ*epoch
    iters_per_epoch = int(2245/32)
    ...
    target_train_loader = torch.utils.data.DataLoader(
        TBNDataSet(args.dataset,
                   pd.read_pickle(args.target_path),
                   data_length,
                   args.modality,
                   image_tmpl,
                   visual_path=args.target_visual_path,
                   audio_path=args.audio_path,
                   num_segments=args.num_segments,
                   transform=train_transform,
                   resampling_rate=args.resampling_rate),
    ...
    for epoch in range(args.start_epoch, args.epochs):
        scheduler.step()
        # train for one epoch
        training_metrics = train(source_train_loader,target_train_loader, model, criterion, optimizer, epoch, device)
        if args.save_stats:
...
def train(train_loader,target_loader, model, criterion, optimizer, epoch, device):
    ...
    for step in range(iters_per_epoch):
        ...
        for m in args.modality:
            #print(type(input[m]))
            input_s[m] = input_s[m].to(device)
            input_t[m] = input_t[m].to(device)

        # compute output
        #target domain
        output_pt,domain_pt = model(input_t)
        #source domain
        output,domain_ps = model(input_s)

        batch_size = input_s[args.modality[0]].size(0)
        ...
        # domain label
        domain_s = Variable(torch.zeros(domain_ps.size(0)).long().cuda())
        # global alignment loss
        dloss_s = 0.5*FL(domain_ps, domain_s)
        
        domain_t = Variable(torch.ones(domain_pt.size(0)).long().cuda())
        dloss_t = 0.5*FL(domain_pt, domain_t)
    ...
    # #target 正解ラベル
    # for i,((input, target),(input_, target_)) in enumerate(zip(train_loader,target_loader)):
    #     #使わない変数
    #     del target_
    ...
    #         input_[m] = input_[m].to(device)
            
    #     # compute output
    #     #target domain
    #     output,domain_pt = model(input_)
    #     #source domain
    #     output,domain_ps = model(input)
